...
export function sessionRoutes(
  ...
  app.group("/api/sessions", (sessions) => {
    ...
    sessions.post(
      ...
      async ({ body, set, token }) => {
        ...
        if (!tokenManager.hasPermission(token, "session:create")) {
          set.status = 403;
          return { error: "Forbidden", message: "Missing permission: session:create" };
        }
        ...
        try {
          ...
          return {
            ...
            message: "Session creation will be implemented in daemon-core (TASK-005)",
          };
        } catch (error) {
          set.status = 500;
          return {
    ...
    sessions.get(
      "/",
      async ({ query, set, token }) => {
        // Check permission
        if (!tokenManager.hasPermission(token, "session:read")) {
          set.status = 403;
          return { error: "Forbidden", message: "Missing permission: session:read" };
        }
    ...
    sessions.get(
      ...
      async ({ params, set, token }) => {
        ...
        try {
          ...
          if (!session) {
            set.status = 404;
            return { error: "Not Found", message: `Session not found: ${sessionId}` };
          }
    ...
    sessions.post(
      ...
      async ({ params, set, token }) => {
        ...
        if (!tokenManager.hasPermission(token, "session:cancel")) {
          set.status = 403;
          return { error: "Forbidden", message: "Missing permission: session:cancel" };
        }
        ...
        try {
          ...
          return {
            error: "Not Implemented",
            message: "Session cancellation will be implemented in daemon-core (TASK-005)",
          };
        } catch (error) {
          set.status = 500;
          return {
    ...
    sessions.post(
      ...
      async ({ params, set, token }) => {
        ...
        try {
          ...
          return {
            error: "Not Implemented",
            message: "Session pause will be implemented in daemon-core (TASK-005)",
          };
        } catch (error) {
          set.status = 500;
          return {
    ...
    sessions.post(
      ...
      async ({ params, set, token }) => {
        ...
        try {
          ...
          return {
            error: "Not Implemented",
            message: "Session resume will be implemented in daemon-core (TASK-005)",
          };
        } catch (error) {
          set.status = 500;
          return {
